...
def compute_significance_and_cohens(clear_rhos, adv_rhos):
    ...
    # Welch's t-test on raw distributions
    t_stat, p_val = stats.ttest_ind(clear_rhos, adv_rhos, equal_var=False)
    
    # Cohen's d Effect Size on raw distributions
...
def evaluate_streaming_lidar(base_dir="/home/avnish/radar_camera_PR/outputs/cmgc_lidar_dinov2/"):
    ...
    # Decoupled seeds for strict reproducibility
    ...
    for f_path in tqdm(clear_files, desc="Extracting Train Patches"):
        ...
        for s in frames:
            if s['scene_token'] in train_set:
                V = np.array(s['lidar_patch_features'], dtype=np.float32)
                depth_mean = np.array(s['lidar_depth_means'], dtype=np.float32)
                
                if len(V) == 0 or len(depth_mean) == 0: continue
                
                depth_std_raw = s.get('lidar_depth_stds', None)
                if depth_std_raw is None:
                    print(f"  [WARN] 'lidar_depth_stds' missing for token {s.get('sample_token', '?')}")
                    depth_std_raw = np.zeros_like(depth_mean)
                depth_std = np.array(depth_std_raw, dtype=np.float32)
                
                log_count_raw = s.get('lidar_log_counts', None)
                if log_count_raw is None:
                    print(f"  [WARN] 'lidar_log_counts' missing for token {s.get('sample_token', '?')}")
                    log_count_raw = np.zeros_like(depth_mean)
                log_count = np.array(log_count_raw, dtype=np.float32)
                
                R = np.column_stack((depth_mean, depth_std, log_count))
                n_patches = len(V)
    ...
    def score_directory(directory, condition_key, is_test_baseline=False):
        ...
        for f_path in tqdm(files, desc=f"Scoring {condition_key.upper()}"):
            ...
            for s in frames:
                ...
                V = np.array(s['lidar_patch_features'], dtype=np.float32)
                depth_mean = np.array(s['lidar_depth_means'], dtype=np.float32)
                if len(V) < 2 or len(depth_mean) == 0: continue
                ...
                # Transform (EFFICIENCY FIX: PCA executed only once)
                R_scaled = scaler_R.transform(R)
                V_pca = pca.transform(V)
                
                V_c, R_c = cca.transform(V_pca, R_scaled)
                
                # Exact Canonical Correlation formula
                delta = V_c[:, 0] - R_c[:, 0]
                rho = float(np.mean(np.clip(1.0 - 0.5 * delta**2, 0.0, 1.0)))
                rhos.append(rho)
                
                # Exact Physics L2 Anomaly Score
                frame_phys_l2 = float(np.mean(np.linalg.norm(R_scaled, axis=1)))
                physics_l2.append(frame_phys_l2)
                
                # SPATIAL SHUFFLE CONTROL
                # Note: rng_shuffle is captured from the outer scope. The sequence of permutations 
                # is deterministic based on the seed, but depends on the order the conditions are scored.
    ...
    results['rain_night_mixed'] = score_directory(extreme_dir, 'night+rain')

    # =========================================================================
    # FINAL STATISTICS & JSON EXPORT
    # =========================================================================
    print("\n" + "="*75)
    print("FINAL RESULTS: DENSE LiDAR BASELINE")
    print("="*75)
    ...
    for condition in ['night', 'rain', 'rain_night_mixed']:
        ...
        # Lower rho is more anomalous -> negate rho
        y_scores_rho = [-x for x in (clear_rhos + adv_rhos)]
        
        # Higher L2 distance is more anomalous -> do NOT negate
